...
import { Cpu, Zap, RotateCcw, Play, Pause, Square, Thermometer, Droplets, Leaf, Sun } from 'lucide-react';
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4 bg-muted/50 rounded-lg px-3 py-2 border border-border flex-wrap gap-2">
        <div className="flex items-center gap-1">
          ...
          <Button variant="ghost" size="icon" className={`h-8 w-8 rounded-full ${simState === 'running' ? 'bg-success/20 text-success' : 'text-muted-foreground'}`} onClick={handleRestart} title="Restart">
            <RotateCcw className="w-4 h-4" />
          </Button>
          <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full text-muted-foreground hover:text-foreground" onClick={handleStop} title="Stop">
